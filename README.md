...
core/        Host-agnostic Flue-core seams (no Cloudflare imports):
             ...
             egress/secret broker interface, deterministic acceptance oracle.
...
```bash
node scripts/acceptance.mjs          # against the default deployed URLs
A_URL=... B_URL=... node scripts/acceptance.mjs
```

Drives the **deterministic core** (the bounded `/sessions/:id/oracle` route) so the A/B
comparison is isolated from LLM nondeterminism. Covers: C1 (A is OOTB/static, no ingest),
C2 (B per-session bearers + tenant tags), C3 (single source of truth — A image == bundle ==
B reconstructed, byte-identical), C4 (same result A vs B — `opening-times.js` stdout
byte-for-byte, plus the injected bearer reaching the echo upstream while the container sends
none), C5 (uniqueness guard + fail-closed egress), plus clean-base and hostile-bundle checks.

## Verified results

All 21 acceptance checks pass against the deployed Workers, and both backends drive the LLM
...
## The `/sessions/:id/oracle` route

A **bounded** test affordance: it runs one of a fixed set of deterministic commands
(`opening-times`, `hash-skill`, `count-skill-files`) built server-side from strictly
validated structured params — **not** arbitrary shell. It exists to drive the acceptance
oracle; it is not a product route.
